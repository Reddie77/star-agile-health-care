...
steps{
  ...
  sh 'minikube start'
  sh 'sleep 30'
  sh 'sudo scp -o StrictHostKeyChecking=no -i ./terraform_files/mumbai.pem deployment.yml ubuntu@172.31.11.120:/home/ubuntu/'
  sh 'sudo scp -o StrictHostKeyChecking=no -i ./terraform_files/mumbai.pem service.yml ubuntu@172.31.11.120:/home/ubuntu/'
script{
  try{
  sh 'ssh -o StrictHostKeyChecking=no -i ./terraform_files/mumbai.pem ubuntu@172.31.11.120 kubectl apply -f .'
  }catch(error)
  {
  sh 'ssh -o StrictHostKeyChecking=no -i ./terraform_files/mumbai.pem ubuntu@172.31.11.120 kubectl apply -f .'
  }
